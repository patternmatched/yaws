...
test_valid_opening_handshake() ->
    ...
    ?line {ok, [Frame]} = wsflush(Sock),
    ?line true = is_valid_close_frame(Frame, [?WS_STATUS_NORMAL,none]),
    ?line ok   = close(Sock),
    ok.
...
test_toolong_payload_ping() ->
    ...
    ?line {ok, [Frame]} = wsflush(Sock),
    ?line true = is_valid_close_frame(Frame, [?WS_STATUS_PROTO_ERROR,none]),
    ?line ok   = close(Sock),
    ok.
...
do_test_10_pings(WSPath, BlockSz) ->
    ...
    ?line ok = wsclose(Sock, ?WS_STATUS_NORMAL, <<>>),
    ?line {ok, Frames}  = wsflush(Sock),
    ?line true = lists:all(fun(#frame{payload=P}) -> (P == Payload) end,
                           lists:sublist(Frames, 9)),
    ?line true = is_valid_close_frame(lists:last(Frames),
                                      [?WS_STATUS_NORMAL,none]),
    ?line ok   = close(Sock),
    ok.
...
test_badrsv_complex(BlockSz) ->
    ...
    ?line {ok, [Frame1, Frame2]} = wsflush(Sock),
    ?line ?WS_OPCODE_TEXT = Frame1#frame.opcode,
    ?line Payload = Frame1#frame.payload,
    ?line true    = is_valid_close_frame(Frame2, [?WS_STATUS_PROTO_ERROR,none]),
    ?line ok      = close(Sock),
    ok.
...
do_test_fragmented_empty(WSPath) ->
    ...
    ?line true = is_valid_close_frame(Frame2, [?WS_STATUS_NORMAL,none]),
    ?line ok   = close(Sock).
...
do_test_unfragmented_invalid_utf8(WSPath, BlockSz) ->
    ...
    Fun = fun(Payload) ->
                  ...
                  ?line {ok, [Frame]} = wsflush(Sock),
                  ?line true = is_valid_close_frame(Frame, [?WS_STATUS_INVALID_PAYLOAD,none]),
                  ?line ok   = close(Sock)
          end,
...
do_test_close_valid_code(WSPath, Code) ->
    ...
    ?line {ok, [Frame]} = wsflush(Sock),
    ?line true = is_valid_close_frame(Frame, [Code,none]),
    ?line ok   = close(Sock),
    ok.
    ...
    case read_frame(Sock) of
        {ok, Frame} ->
            case Frame#frame.opcode of
                ?WS_OPCODE_CLOSE -> {ok, lists:reverse([Frame|Acc])};
                _                -> wsflush(Sock, [Frame|Acc])
            end;
        {error, closed} ->
            {ok, lists:reverse([none|Acc])};
        {error, Reason} ->
            {error, Reason}
    end.
...
%% ----
is_valid_close_frame(none, Codes) ->
    io:format("    WARNING: No close frame~n", []),
    lists:member(none, Codes);
is_valid_close_frame(#frame{opcode=?WS_OPCODE_CLOSE,
                            payload = <<>>},
                     Codes) ->
    lists:member(?WS_STATUS_NORMAL, Codes);
is_valid_close_frame(#frame{opcode=?WS_OPCODE_CLOSE,
                            payload = <<Status:16/big, _/binary>>},
                     Codes) ->
    lists:member(Status, Codes);
is_valid_close_frame(_, _) ->
    false.
